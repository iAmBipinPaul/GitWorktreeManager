...
public class GitServiceIntegrationTests : IAsyncLifetime
{
    ...
    public async Task InitializeAsync()
    {
        ...
        string readmePath = Path.Combine(_testRepoPath, "README.md");
    ...
    {
        // Act
        bool result = await _gitService.IsGitInstalledAsync();
    ...
    {
        // Act
        string? result = await _gitService.GetRepositoryRootAsync(_testRepoPath);
        ...
        string normalizedExpected = Path.GetFullPath(_testRepoPath)
            .TrimEnd(Path.DirectorySeparatorChar, Path.AltDirectorySeparatorChar);
    ...
    public async Task GetRepositoryRootAsync_WithNonRepository_ReturnsNull()
    {
        ...
        // Act
        string? result = await _gitService.GetRepositoryRootAsync(nonRepoPath);
    ...
    public async Task FullWorktreeWorkflow_AddListRemove_WorksCorrectly()
    {
        ...
        GitCommandResult<IReadOnlyList<Worktree>> initialList = await _gitService.GetWorktreesAsync(_testRepoPath);
        ...
        GitCommandResult addResult = await _gitService.AddWorktreeAsync(
            _testRepoPath,
            _worktreePath,
            "test-branch",
            false);
        ...
        GitCommandResult<IReadOnlyList<Worktree>> afterAddList = await _gitService.GetWorktreesAsync(_testRepoPath);
        ...
        Worktree? newWorktree = afterAddList.Data.FirstOrDefault(w => !w.IsMainWorktree);
        ...
        GitCommandResult removeResult = await _gitService.RemoveWorktreeAsync(
            _testRepoPath,
            _worktreePath,
            false);
        ...
        GitCommandResult<IReadOnlyList<Worktree>> afterRemoveList = await _gitService.GetWorktreesAsync(_testRepoPath);
    ...
    {
        // Arrange
        string newBranchWorktreePath =
            Path.Combine(Path.GetTempPath(), $"GitWorktreeTest_NewBranch_{Guid.NewGuid():N}");
        ...
        try
        {
            ...
            // Assert
            addResult.Success.Should()
                .BeTrue($"Adding worktree with new branch should succeed. Error: {addResult.ErrorMessage}");
            Directory.Exists(newBranchWorktreePath).Should().BeTrue();

            // Verify the branch was created
            GitCommandResult<IReadOnlyList<Worktree>> listResult = await _gitService.GetWorktreesAsync(_testRepoPath);
            listResult.Data.Should().Contain(w => w.Branch == "new-feature-branch");
        }
        finally
        {
            // Cleanup
            await _gitService.RemoveWorktreeAsync(_testRepoPath, newBranchWorktreePath, true);
    ...
    {
        // Arrange
        string failWorktreePath = Path.Combine(Path.GetTempPath(), $"GitWorktreeTest_Fail_{Guid.NewGuid():N}");
    ...
    {
        // Arrange
        string nonExistentPath = Path.Combine(Path.GetTempPath(), $"NonExistent_{Guid.NewGuid():N}");
    ...
    {
        // Act
        GitCommandResult<IReadOnlyList<Worktree>> result = await _gitService.GetWorktreesAsync(_testRepoPath);

        // Assert
        result.Success.Should().BeTrue();
        result.Data.Should().NotBeNull();

        Worktree mainWorktree = result.Data!.First();
